...
- **`pixKey`**: A chave PIX da subconta (Não precisa ser necessariamente atrelada a uma conta da woovi).
...
```sh
  curl 'https://api.woovi.com.br/api/v1/subaccount' -X POST \
      -H "Accept: application/json" \
      -H "Content-Type: application/json" \
      -H "Authorization": "app_id"
      --data-binary '{"name":"subaccount#1","pixKey": "destinatario@woovi.com.br"}'
```
...
```js
fetch('https://api.woovi.com.br/api/v1/subaccount', {
  method: 'POST',
  body: JSON.stringify({
    name: "subaccount#1",
