...
O [link de onboarding](./api-onboarding-create.mdx) é o caminho mais curto: você cria o onboarding e o seu cliente preenche tudo numa página hospedada pela Woovi. Quando a sua integração BAAS já tem o próprio funil — coleta os documentos, a selfie e os dados dos sócios dentro do seu produto — dá para fazer **o onboarding inteiro pela API**, sem que o cliente final veja nenhuma tela da Woovi.
...
:::info Pré-requisitos
- Empresa com as features **BAAS** e **KYC_ONBOARDING_LINK** habilitadas.
...
| `STATUS_NOT_SUBMITTABLE` | O cadastro não está em `DRAFT`/`PENDING` | Consulte o status; não há o que reenviar |
...
| `403` | — | Empresa sem BAAS, sem `PIX_AUTHENTICATION_KYC` (Pix), ou AppID sem o escopo |
