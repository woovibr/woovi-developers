...
## `ACCOUNT_REGISTER_STEP_UPDATED` {#account-register-step-updated}
...
## `ACCOUNT_REGISTER_IN_REVIEW` {#account-register-in-review}
...
## `ACCOUNT_REGISTER_DOCUMENTS_REQUESTED` {#account-register-documents-requested}
...
## `ACCOUNT_REGISTER_RFI_RESOLVED` {#account-register-rfi-resolved}
...
:::note Só uma devolução marca uma retentativa
`retrying` sai de uma marca gravada no cadastro quando a RFI invalida o passo, e essa marca
é consumida no momento em que o passo é reconcluído. Duas consequências práticas:

- **é por passo, nunca pelo cadastro inteiro.** Uma devolução que pediu só o contrato social
  não marca `TERMS` como retentativa quando o cliente passa por ele de novo;
- **refazer um passo sem uma nova devolução não emite nada.** O evento sai só na transição
  de pendente para concluído: reenviar o mesmo documento, ou salvar o mesmo formulário duas
  vezes, não gera um segundo `STEP_UPDATED`.
:::
