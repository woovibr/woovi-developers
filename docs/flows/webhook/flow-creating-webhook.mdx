...
payload {
  charge: {
    ...
    brCode: 'https://api.woovi.com/openpix/openpix/testing?transactionID=ea83401ed4834b3ea6f1f283b389af29'
  },
  pix: {
    charge: {
      status: 'COMPLETED',
...
```jsx
curl --location --request POST 'https://api.woovi.com/openpix/api/openpix/v1/webhook' \
...
const createWebhook = async () => {
  ...
  const response = await fetch('http://api.woovi.com/api/openpix/v1/webhook', {
    method: 'POST',
    headers: {
...
- authorization (opcional): auth key para validação na Woovi em sua API. Campo opcional, porém indicamos que o mesmo exista.
