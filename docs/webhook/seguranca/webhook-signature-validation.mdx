...
title: Validando Webhook payload usando x-webhook-signature
tags:
  - webhook
  - security
  - signature
...
Toda request do Webhook possui o header `x-webhook-signature` que é a assinatura gerada com a **chave privada da Woovi** e o payload do Webhook.

## Qual a diferença entre x-webhook-signature e X-OpenPix-Signature (HMAC)?
...
- **x-webhook-signature**: Use este método para confirmar que o webhook realmente foi enviado pela Woovi. Todos os webhooks da Woovi usam a mesma chave pública para validação.

- **X-OpenPix-Signature (HMAC)**: Este é o **método mais seguro**. Ele valida que o webhook veio da sua configuração específica de webhook, pois utiliza uma secret key única que só você e a Woovi conhecem. [Saiba mais sobre validação HMAC](./webhook-hmac).

:::tip Recomendação
Para maior segurança, recomendamos utilizar o método **HMAC (X-OpenPix-Signature)**, pois garante que o webhook veio especificamente da sua configuração de webhook, e não apenas de qualquer conta Woovi.
:::

## Chave Pública da Woovi

Para validar o `x-webhook-signature`, você deve usar a **chave pública da Woovi** (em Base64):
...
Esta chave é a mesma para todos os webhooks da Woovi.
