// AUTOGENERATED by @woovi/openapi scripts/generateApiExplorerData.ts — do not edit by hand.
// Source: woovi-openapi build/<engine>/woovi.json
// Regenerate with: pnpm --filter @woovi/openapi copy-to-developers
...
const endpoints: ApiEndpoint[] = [
  ...
  {
    'id': 'post-api-v1-anticipation-id-approve',
    'method': 'POST',
    'path': '/api/v1/anticipation/{id}/approve',
    'tag': 'anticipation',
  ...
  {
    'id': 'post-api-v1-anticipation-balance-batch',
    'method': 'POST',
  ...
  {
    'id': 'post-api-v1-anticipation-beneficiary',
    'method': 'POST',
  ...
  {
    'id': 'post-api-v1-anticipation-id-reject',
    'method': 'POST',
    'path': '/api/v1/anticipation/{id}/reject',
    'tag': 'anticipation',
    'category': 'anticipation',
    ...
    'responseExamples': [
      {
        ...
        'summary': 'A settled boleto IN and a boleto OUT',
      },
    ],
  },
  {
    'id': 'post-api-v1-boleto-validate',
    'method': 'POST',
    'path': '/api/v1/boleto/validate',
    'tag': 'boleto',
    'category': 'boleto',
    'summary': 'Validate a boleto by barcode',
    'description': 'Validates a boleto by its barcode before paying it. This is step 1 of the\nBoleto OUT flow: it confirms the amount, due date and beneficiary so you can\nreview the boleto before creating the payment.\n\nThe barcode must have 44, 47 or 48 digits. All monetary values are returned in cents.\n\nRequires the `BOLETO_VALIDATE_POST` scope on the application.\n\nAfter validating, create the payment with `POST /api/v1/payment` using\n`type: "BOLETO"` and the `boletoBarcode`.\n',
    'requestExamples': [
      {
        'name': 'Barcode',
        'value': {
  ...
  {
    'id': 'get-api-v1-funds-recovery-id-disputes',
    'method': 'GET',
    'path': '/api/v1/funds-recovery/{id}/disputes',
    'tag': 'funds recovery',
    'category': 'Recuperação de fundos (MED)',
    'summary': 'List the disputes related to a funds recovery (MED)',
    'description': 'Returns the disputes (MED) of your account that are related to a funds recovery. Only disputes that belong to your account are returned.\n\nRequires the **MED API** feature enabled on your account.\n',
    'requestExamples': [],
    'responseExamples': [
      {
        'name': 'default',
  ...
  {
    'id': 'get-api-v1-funds-recovery-id-infraction-reports',
    'method': 'GET',
    'path': '/api/v1/funds-recovery/{id}/infraction-reports',
    'tag': 'funds recovery',
    'category': 'Recuperação de fundos (MED)',
    'summary': 'List the infraction reports of a funds recovery (MED)',
    'description': 'Returns the infraction reports (fraud notifications opened with the Central Bank) linked to the funds recovery, for infraction reports that belong to your account.\n\nRequires the **MED API** feature enabled on your account.\n',
    'requestExamples': [],
    'responseExamples': [
      {
        'name': 'default',
        'value': [
          {
            'bacenInfractionReportId': '7a2f3c1a-8b4d-4e21-9f30-2c1a3b4d5e6f',
            'fundsRecoveryId': '3e760cd5-39b2-45da-8ab6-b212cf205568',
            ...
            'fraudMarkerId': 'a1b2c3d4-e5f6-4789-9abc-def012345678',
            'analysisResult': 'AGREED',
            'analysisDetails': 'Fraud confirmed by the counterparty analysis',
  ...
  {
    'id': 'get-api-v1-funds-recovery-id-refund-solicitations',
    'method': 'GET',
    'path': '/api/v1/funds-recovery/{id}/refund-solicitations',
    'tag': 'funds recovery',
    'category': 'Recuperação de fundos (MED)',
    'summary': 'List the refund solicitations of a funds recovery (MED)',
    'description': 'Returns every refund solicitation the Central Bank opened on the legs of a funds recovery. Use it to follow, per leg, the `status`, the requested `refundAmount` and the amount effectively returned in `EffectiveRefundedAmount`.\n\nRequires the **MED API** feature enabled on your account.\n',
    'requestExamples': [],
    'responseExamples': [
      {
        'name': 'default',
        'value': [
          {
            ...
            'refundedAt': '2026-06-12T00:05:00.000Z',
            'refundTransaction': {
              'endToEndId': 'D31680151202606120000AbCdEf67890',
              'value': 32000,
              'date': '2026-06-12T00:05:00.000Z',
            },
            'status': 'CLOSED',
    ...
    'requestExamples': [
      ...
      },
      {
        ...
        'value': {
          ...
          'comment': 'boleto payment',
          'correlationID': 'boleto-out-1',
        },
        'summary': 'Pay a boleto (Boleto OUT)',
      },
      {
        'name': 'boletoAutoApprove',
        'value': {
          'type': 'BOLETO',
          'boletoBarcode': '34195148200000003001095517077320772982609000',
          'comment': 'boleto payment',
          'correlationID': 'boleto-out-auto-approve',
          'autoApprove': true,
        },
        'summary': 'Pay a boleto with auto-approval',
      },
    ],
    'responseExamples': [
      {
      ...
      },
      {
        ...
        'summary': 'Boleto payment response',
      },
      {
        'name': 'autoApproved',
        'value': {
  ...
  },
  {
    ...
    'description': "For USD input, send `inputCurrency: USD`, `inputPaymentMethod: WIRE` or `ACH`,\n`value` in integer USD cents, `currency: USDC` or `USDT`, and an explicit\n`destinationWalletAddress` on a supported network. `correlationId` is mandatory.\nThe AppID bank account must own a confirmed subaccount whose USD rail is unlocked.\n`grossAmount` is not accepted for USD. The response includes `usdDepositInstructions`\nwith the bank details and reference to use when sending the bank transfer. Never\ncall `/deposit/approve` for USD: no Woovi BRL account is debited. Provider fees are\nreported in `quote.appliedFees` with their currencies; BRL fee fields do not apply.\nStaging storage/upload is supported, but Avenia sandbox does not unlock USD rails.\n\nRetrying the same USD `correlationId` and parameters returns the existing deposit\nwithout creating another ticket. HTTP 202 with no instructions means the original\nticket result needs reconciliation; keep the correlationId and contact support,\nrather than starting another transfer. Settlement arrives through deposit webhooks.\n\nWith inputCurrency omitted (or BRL), creates a PIX-in to stable-out deposit. The deposit\nconverts a BRL amount (in cents) into the requested stablecoin on the chosen network\nand returns a quote with the applied fees.\n\nThere are two fee models, and the amount field you send picks one:\n  - `grossAmount` (recommended): all-in. The Woovi fee comes OUT of it, so the account\n    is debited exactly `grossAmount` and the provider is paid the remainder.\n  - `value` (legacy): the amount paid to the provider, with the Woovi fee charged ON\n    TOP — the account is debited `value` + `wooviFee`.\n\nEither way the response breaks the total down into `wooviFee` and `providerFee`, and\n`GET /api/v1/stablecoin/quote` reports the same numbers for the same input.\n\nFor BRL input, the deposit uses the stable subaccount of the CompanyBankAccount\nlinked to the authenticated AppID. USD is delivered to the explicit destination wallet. The AppID must have a `companyBankAccount`\nconfigured (otherwise `400 APPLICATION_COMPANY_BANK_ACCOUNT_MISSING`), and that bank\naccount must have a stable subaccount in `CONFIRMED` status (a completed KYB) —\nanother bank account's subaccount on the same company is never used. Otherwise the\nrequest is rejected with a `400`.\n\n`subAccountId`, when sent, must belong to that same CompanyBankAccount.\n\nNot every asset is available on every network. The supported matrix is:\n  - USDT: POLYGON, ETHEREUM, CELO, TRON, BNB\n  - USDC: POLYGON, ETHEREUM, BASE, CELO, BNB\n  - BRLA: POLYGON, ETHEREUM, BASE, CELO\n\nIf `network` is omitted it defaults to `POLYGON`. Sending an asset/network combination\noutside the matrix above returns a `400`.\n\nIdempotency is supported via `correlationId`.\n",
    'requestExamples': [
      {
        'name': 'UsdWire',
        'value': {
        ...
        'summary': 'Deposit USD by WIRE and deliver USDC on Base',
      },
      {
        'name': 'AllInRequest',
        'value': {
